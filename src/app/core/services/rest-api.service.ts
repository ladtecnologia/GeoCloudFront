...
import { Contacts } from 'src/app/pages/crm/contacts/data';
import { Deposit } from 'src/app/models/Deposit';
import { ContactsModel } from 'src/app/pages/crm/contacts/contacts.model';
...
export class restApiService {
  ...
  */
  ...
  getDepositData(): Observable<Deposit[]> {
    return this.http.get<Deposit[]>(GlobalComponent.baseURL+'Deposit/get').pipe(take(1));
  }

  getSingleDepositData(id:number): Observable<any> {
    return this.http.get(`${GlobalComponent.baseURL}Deposit/getById?id=${id}`).pipe(take(1));
  }

  deleteDeposit(id: any): Observable<any>{
    return this.http.delete(`${GlobalComponent.baseURL}Deposit/delete?id=${id}`).pipe(take(1));
  }

  postDepositData(deposit: Deposit): Observable<any> {            
    return this.http.post(GlobalComponent.baseURL+'Deposit/add', deposit).pipe(take(1));
  }

  patchDepositData(deposit: Deposit): Observable<Deposit> {    
  ...
  public getUser(): Observable<User[]>{
    return this.http.get<User[]>(GlobalComponent.baseURL+'User/get').pipe(take(1));
  }

  public getUserByFirstName(firstName: string): Observable<User[]>{
    return this.http.get<User[]>(`${GlobalComponent.baseURL}User/getByFirstName?firstName=${firstName}`).pipe(take(1));
  }
